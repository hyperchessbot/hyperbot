...
Warning: Playing while the bot home page is open introduces increased work load on the server. If in addition you use too high values for ENGINE_HASH ( recommended <= 128 ) and ENGINE_THREADS ( recommended <= 4 ), then Heroku rate limits may set in and temporarily disable server, which will prevent the bot from making moves. If your priority is high rating or your bot is playing in bot tournaments, then close the bot homepage while playing. If your bot fails to move look at the Heroku logs, and if you get memory quota exceeded error, then decrease ENGINE_THREADS and ENGINE_HASH.
			</p>
			</div>
            <script>            
